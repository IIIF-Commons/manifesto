...
export class LabelValuePair {
    ...
    public setLabel(value: string): void {
        if (this.label && this.label.length) {
            var t: Language = this.label.filter(x => x.locale === this.defaultLocale || x.locale === Utils.getInexactLocale(this.defaultLocale))[0];
            if (t) t.value = value;
        }
    }
    ...
    public setValue(value: string): void {
        if (this.value && this.value.length) {
            var t: Language = this.value.filter(x => x.locale === this.defaultLocale || x.locale === Utils.getInexactLocale(this.defaultLocale))[0];
            if (t) t.value = value;
        }
    }
}