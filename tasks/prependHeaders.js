...
gulp.task('prependHeaders', function(cb){
    Promise.all([
        ...
        utils.prependHeader(config.header, config.dist + '/' + config.dtsBundleOut, config.dist),
        utils.prependHeader(config.header, config.client + '/' + config.jsOut, config.client),
        utils.prependHeader(config.header, config.client + '/' + config.jsMinOut, config.client),
        utils.prependHeader(config.header, config.server + '/' + config.jsOut, config.server),
        utils.prependHeader(config.header, config.server + '/' + config.jsMinOut, config.server)
    ]).then(function(){
        cb();
    });
});